...
exports.Aranet4Platform = void 0;
const aranet_1 = require("./aranet");
...
class Aranet4Platform {
    constructor(log, config, api) {
        ...
        this.configuredDevicesBySerial = new Map(this.config.devices.map((device) => [device.serialNumber, device]));
        this.log.debug(`Initialized ${settings_1.PLUGIN_NAME}: ${JSON.stringify(this.config)}`);
    ...
    async discoverDevices() {
        ...
        let devices;
        try {
            devices = await aranet_1.Aranet4Device.getAranet4Devices(this.log, this.config.bluetoothReadyTimeout, this.config.bluetoothDeviceSearchTimeout, {
        ...
        for (const configuredDevice of configuredDevices) {
            if (!foundSerialNumbers.has(configuredDevice.serialNumber)) {
                this.log.warn(`Configured Aranet4 ${configuredDevice.serialNumber} (${configuredDevice.name}) was not found during scan`);
            }
        }
        for (const device of managedDevices) {
            const configuredDevice = this.configuredDevicesBySerial.get(device.info.serialNumber);
            const accessoryName = (configuredDevice === null || configuredDevice === void 0 ? void 0 : configuredDevice.name) || device.info.modelNumber;
            ...
            if (existingAccessory) {
                ...
                new platformAccessory_1.Aranet4Accessory(this, existingAccessory, device);
            ...
            new platformAccessory_1.Aranet4Accessory(this, accessory, device);
            this.api.registerPlatformAccessories(settings_1.PLUGIN_NAME, settings_1.PLATFORM_NAME, [accessory]);
        }
    }
    normalizeDeviceConfigs(devices) {
    ...
    getEnabledConfiguredDevices() {
        return this.config.devices.filter((device) => device.enabled);
    }
    removeUnconfiguredCachedAccessories() {
        if (this.config.autoDiscover || !this.configuredDevicesBySerial.size) {
            return;
        }
        const enabledSerialNumbers = new Set(this.getEnabledConfiguredDevices().map((device) => device.serialNumber));
        const staleAccessories = this.accessories.filter((accessory) => {
            var _a, _b;
            const serialNumber = (_b = (_a = accessory.context) === null || _a === void 0 ? void 0 : _a.device) === null || _b === void 0 ? void 0 : _b.serialNumber;
        ...
        this.accessories.splice(0, this.accessories.length, ...this.accessories.filter((accessory) => !staleAccessories.includes(accessory)));
    }
    syncConfiguredCachedAccessories() {
        var _a, _b;
    ...
    renameCachedService(accessory, ServiceType, name) {
...
exports.Aranet4Platform = Aranet4Platform;
