...
exports.Aranet4Accessory = void 0;
class Aranet4Accessory {
    constructor(platform, accessory, device) {
        var _a, _b;
        this.platform = platform;
        this.accessory = accessory;
        this.device = device;
        this.name = device.info.name || ((_b = (_a = this.accessory.context) === null || _a === void 0 ? void 0 : _a.device) === null || _b === void 0 ? void 0 : _b.name) || 'Aranet4';
        this.serviceNames = {
        ...
        this.humidityService = this.getOrAddPrimaryService(this.platform.Service.HumiditySensor, this.serviceNames.humidity);
        this.temperatureService = this.getOrAddPrimaryService(this.platform.Service.TemperatureSensor, this.serviceNames.temperature);
        this.co2Service = this.getOrAddPrimaryService(this.platform.Service.CarbonDioxideSensor, this.serviceNames.co2);
    ...
    getOrAddPrimaryService(ServiceType, name) {
    ...
    removeOptionalService(ServiceType, name, subtype) {
    ...
    removeDuplicatePrimaryServices(ServiceType, serviceToKeep) {
        ...
        for (const duplicateService of duplicateServices) {
            this.platform.log.warn(`Removing duplicate cached Aranet4 service: ${duplicateService.displayName || ServiceType.name}`);
            this.accessory.removeService(duplicateService);
        }
    }
    removeUnsupportedConfiguredName(service) {
        const serviceWithConfiguredName = service;
        const ConfiguredName = this.platform.Characteristic.ConfiguredName;
        if (typeof serviceWithConfiguredName.testCharacteristic === 'function' &&
    ...
    async updateSensorData() {
        ...
        if (this.co2PpmService) {
            this.co2PpmService.updateCharacteristic(this.platform.Characteristic.CurrentAmbientLightLevel, Math.max(0.0001, data.co2));
        }
        if (this.airQualityService) {
            this.airQualityService.updateCharacteristic(this.platform.Characteristic.AirQuality, this.getAirQuality(data.co2));
        }
        this.platform.log.info(`Updated ${this.name}: CO2 ${data.co2} ppm, temperature ${data.temperature} C, ` +
...
exports.Aranet4Accessory = Aranet4Accessory;
